    ...
    withDb { conn =>
      ...
      pgu.datasetReader.openDataset(copyInfo).run { readCtx =>
        ...
        val (qrySchema, dataVersion, mresult) =
          ds.run { dsInfo =>
            val qs = new QueryServer(dsInfo, caseSensitivity, leadingSearch, Duration.Zero, new analyzer2.ProcessQuery(analyzer2.NoopResultCache, analyzer2.ProcessQuery.TimeoutManager.Noop))
