...
impl OwnedBlowfish {
    /// Create a `OwnedBlowfish` cipher with the given key
...
mod tests {
    ...
    fn owned_agrees_with_reference_implementation() {
        let key = b"1234567890123456789012345678901234567890123456789012345678900123456789012";
        let bf = OwnedBlowfish::new(key);
        let encrypted = bf.encrypt(0x123456789abcdef);
        assert_eq!(encrypted, 0xb1f608faee80ca25);
