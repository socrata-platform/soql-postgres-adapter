...
class PGSecondaryLogger[CT, CV] extends Logger[CT, CV] {
  ...
  def secondaryAddIndex(fieldName: ColumnName, directives: JObject): Unit = logger.debug("secondaryAddIndex: " + fieldName)

  def secondaryDeleteIndex(fieldName: ColumnName): Unit = logger.debug("secondaryDeleteIndex: " + fieldName)
}
