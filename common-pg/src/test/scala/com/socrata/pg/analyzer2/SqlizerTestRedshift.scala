...
class SqlizerTestRedshift extends FunSuite with Matchers with SqlizerUniverse[SqlizerTest.TestMT] with PGSecondaryUniverseTestBase{
  ...
  test("simple") {
    ...
    sqlish should equal ("SELECT x1.text AS i1, x1.num AS i2, x1.compound_a AS i3_a, x1.compound_b AS i3_b FROM table1 AS x1")
  }
  ...
  test("simple-positions") {
    ...
    sqlish should equal ("SELECT (x1.num) * (2.0 :: numeric) AS i1 FROM table1 AS x1 ORDER BY (x1.num) * (2.0 :: numeric) ASC NULLS LAST")
    ...
    sqlish.length should equal(positions.length) // just a sanity check

    poses.length should equal (sqlish.length)
    poses.zip(positions).foreach {
      case (NoPosition, '_') => // ok
      case (p, 'A') => (p.line, p.column) should equal ((1, 8))
      case (p, 'B') => (p.line, p.column) should equal ((1, 14))
  ...
  test("table op - no compression") {
    ...
    sqlish should equal ("(SELECT x1.num AS i1, x1.compound_a AS i2_a, x1.compound_b AS i2_b FROM table1 AS x1) UNION (SELECT x3.number AS i3, x3.other_compound_a AS i4_a, x3.other_compound_b AS i4_b FROM table2 AS x3)")
  }
  ...
  test("table op - left compression induces right compression") {
    ...
    sqlish should equal ("(SELECT x1.num AS i1, soql_compress_compound(x1.compound_a, x1.compound_b) AS i2 FROM table1 AS x1) UNION (SELECT g1.i3 AS i3, soql_compress_compound(g1.i4_a, g1.i4_b) AS i4 FROM (SELECT x3.number AS i3, x3.other_compound_a AS i4_a, x3.other_compound_b AS i4_b FROM table2 AS x3) AS g1)")
  }
  ...
  test("table op - right compression induces left compression") {
    ...
    sqlish should equal ("(SELECT g1.i1 AS i1, soql_compress_compound(g1.i2_a, g1.i2_b) AS i2 FROM (SELECT x1.num AS i1, x1.compound_a AS i2_a, x1.compound_b AS i2_b FROM table1 AS x1) AS g1) UNION (SELECT x3.number AS i3, soql_compress_compound(x3.other_compound_a, x3.other_compound_b) AS i4 FROM table2 AS x3)")
  }
  ...
  test("table op - both compression induces nothing compression") {
    ...
    sqlish should equal ("(SELECT x1.num AS i1, soql_compress_compound(x1.compound_a, x1.compound_b) AS i2 FROM table1 AS x1) UNION (SELECT x3.number AS i3, soql_compress_compound(x3.other_compound_a, x3.other_compound_b) AS i4 FROM table2 AS x3)")
  }
  ...
  test("provenance - simple") {
    ...
    val sqlish = analyze(tf, soql).layoutSingleLine.toString
    sqlish should equal ("SELECT text \"table1\" AS i1_provenance, x1.:id AS i1 FROM table1 AS x1 WHERE (x1.:id) = (1200459281559959 :: bigint)")
  }
  ...
  test("provenance - joined") {
    ...
    val sqlish = analyze(tf, soql).layoutSingleLine.toString
    sqlish should equal ("SELECT text \"table1\" AS i1_provenance, x1.:id AS i1 FROM table1 AS x1 JOIN table2 AS x2 ON ((text \"table1\") IS NOT DISTINCT FROM (text \"table2\")) AND ((x1.:id) = (x2.:id))")
  }
  ...
  test("provenance - compressed") {
    ...
    val sqlish = analyze(tf, soql).layoutSingleLine.toString
    sqlish should equal ("SELECT text \"table1\" AS i1_provenance, x1.:id AS i1 FROM table1 AS x1 WHERE (soql_compress_compound(text \"table1\", x1.:id)) = (soql_compress_compound(text \"table1\", 1200459281559959 :: bigint))")
  }
  ...
  test("provenance - order by physical column does not include the provenance") {
    ...
    val sqlish = analyze(tf, soql).layoutSingleLine.toString
    sqlish should equal ("SELECT text \"table1\" AS i1_provenance, x1.:id AS i1 FROM table1 AS x1 ORDER BY x1.:id ASC NULLS LAST")
  }
  ...
  test("provenance - order by simple logical column does not include the provenance") {
    ...
    val sqlish = analyze(tf, soql).layoutSingleLine.toString
    sqlish should equal ("SELECT x2.i1_provenance AS i2_provenance, x2.i1 AS i2 FROM (SELECT text \"table1\" AS i1_provenance, x1.:id AS i1 FROM table1 AS x1) AS x2 ORDER BY x2.i1 ASC NULLS LAST")
  }
  ...
  test("provenance - order by unioned logical column DOES include the provenance") {
    ...
    val sqlish = analyze(tf, soql).layoutSingleLine.toString
    sqlish should equal ("SELECT x5.i1_provenance AS i3_provenance, x5.i1 AS i3 FROM ((SELECT text \"table1\" AS i1_provenance, x1.:id AS i1 FROM table1 AS x1) UNION (SELECT text \"table2\" AS i2_provenance, x3.:id AS i2 FROM table2 AS x3)) AS x5 ORDER BY x5.i1_provenance ASC NULLS LAST, x5.i1 ASC NULLS LAST")
  }
  ...
  test("table function - trivial") {
    ...
    val sqlish = analyze(tf, soql).layoutSingleLine.toString
    sqlish should equal ("SELECT windowed_function(x1.txt) OVER () AS i1 FROM table1 AS x1")
  }
  ...
  test("table function - no bounds") {
    ...
    val sqlish = analyze(tf, soql).layoutSingleLine.toString
    sqlish should equal ("SELECT windowed_function(x1.txt) OVER (PARTITION BY x1.category ORDER BY x1.txt ASC NULLS LAST) AS i1 FROM table1 AS x1")
  }
  ...
  test("table function - single bound") {
    ...
    val sqlish = analyze(tf, soql).layoutSingleLine.toString
    sqlish should equal ("SELECT windowed_function(x1.txt) OVER (PARTITION BY x1.category ORDER BY x1.txt ASC NULLS LAST ROWS 5 PRECEDING) AS i1 FROM table1 AS x1")
  }
  ...
  test("table function - double single bound") {
    ...
    val sqlish = analyze(tf, soql).layoutSingleLine.toString
    sqlish should equal ("SELECT windowed_function(x1.txt) OVER (PARTITION BY x1.category ORDER BY x1.txt ASC NULLS LAST ROWS BETWEEN 5 PRECEDING AND 5 FOLLOWING) AS i1 FROM table1 AS x1")
  }
}
