...
class SqlizerBasicTest extends SqlizerTest {
  ...
  test("select aggregate functions") {
    ...
    val ParametricSql(Seq(sql), setParams) = sqlize(soql, CaseSensitive)
    sql should be ("""SELECT ((count("t1".id))::numeric),(avg("t1".id)),(min("t1".id)),(max("t1".id)),(sum("t1".id)),((percentile_cont(.50) within group (order by "t1".id)) :: NUMERIC),(percentile_disc(.50) within group (order by "t1".case_number)) FROM t1""")
    setParams.length should be (0)
  }
